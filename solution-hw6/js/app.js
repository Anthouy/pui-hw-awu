...
//parseInt(element['basePrice'].replace("$","")).toFixed(2)
//parseFloat(element['basePrice'].replace("$",""))
...
function retrieveFromLocalStorage() {
  ...
  rollArray.forEach(element => {
    console.log(element);
    const roll = addNewRoll(element['type'], element['glazing'], Number(element['size']), parseFloat(element['basePrice'].replace("$","")).toFixed(2));
    createElement(roll);
  })
  console.log(rollSet)
}
